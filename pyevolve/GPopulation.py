   ...
   **Examples**
   ...
   def sort(self):
      ...
      if self.sortType == Consts.sortType["raw"]:
         ...
         self.internalPop.sort(key=attrgetter('score'), reverse=rev)
      else:
         self.scale()
         # self.internalPop.sort(cmp=Util.cmp_individual_scaled, reverse=rev)
         self.internalPop.sort(key=attrgetter('fitness'), reverse=rev)
         self.internalPopRaw = self.internalPop[:]
         # self.internalPopRaw.sort(cmp=Util.cmp_individual_raw, reverse=rev)
         self.internalPopRaw.sort(key=attrgetter('score'), reverse=rev)
